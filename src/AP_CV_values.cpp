...
// The class CvValues is defined in AP_CV_values.h
CvValues cvValues;
...
void CvValues::init(uint8_t decoderType, uint8_t softwareVersion) {
  ...
  defaults[DecType] = decoderType;      // See const definitions from AP_CV_values.h
  ...
  switch (decoderType) {
    ...
    case SafetyDecoder:
      ...
      // 
      defaults[T_Watchdog] = 5;         // Number of seconds the watchdog relay will remain active
      defaults[T_TrainMove] = 20;       // Time (in 100ms steps) after an RS-emergency button push the PC gets to stop all trains
